...
object PriorSkills {
  ...
  private def covariance(player1: Player, player2: Player, priorVarOnServe: Double, priorVarOnReturn: Double): Double = {

    val theSameGameVar = if (player1.equals(player2)) 0.0002 else 0.0001

    val playerOverTimeVar = if (player1.playerName.equals(player2.playerName) && player1.skillOnServe == player2.skillOnServe) {

      val sf = if (player1.skillOnServe) priorVarOnServe
