...
case class ScoresSimulator {
  ...
  def simulate(scores: Array[Score], opponentMap: Map[String, OpponentType]): Tuple2[Array[Score], Double] = {
    ...
    val simScores = scoreSim(scores, skillMeanFunc, covFunc, logPerfStdDev = trueParams.data.last).map(s => s.score)
