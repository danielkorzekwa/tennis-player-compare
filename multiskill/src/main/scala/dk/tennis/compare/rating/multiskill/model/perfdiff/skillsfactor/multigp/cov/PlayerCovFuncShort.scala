 ...
 */
case class PlayerCovFuncShort(params: Seq[Double]) extends PlayerCovFunc {

  val logSfShort = params(0)
  val logEllShort = params(1)
  ...
  private def covariance_df_sf(player1: Player, player2: Player, logSf: Double, logEll: Double): Double = {
    ...
    val theSamePlayerCov = if (player1.playerName.equals(player2.playerName)) 1d else 0
    val timeDiffCov = new CovSEiso(sf = logSf, logEll).df_dSf(Matrix(player1.timestamp.getTime.toDouble / DAY_MILLIS), Matrix(player2.timestamp.getTime.toDouble / DAY_MILLIS))
  ...
  private def covariance_df_ell(player1: Player, player2: Player, logSf: Double, logEll: Double): Double = {
    ...
    val theSamePlayerCov = if (player1.playerName.equals(player2.playerName)) 1d else 0
    val timeDiffCov = new CovSEiso(sf = logSf, logEll).df_dEll(Matrix(player1.timestamp.getTime.toDouble / DAY_MILLIS), Matrix(player2.timestamp.getTime.toDouble / DAY_MILLIS))
    onServeCov * theSamePlayerCov * timeDiffCov
  }
