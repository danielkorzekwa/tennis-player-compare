...
class MatchModelTesterTest {
  ...
  val tester = MatchModelTester(matchesFile, 2011, 2011)
  ...
  @Test def test {
    ...
    // val matchFilter = (m: MatchComposite) => {log.info(m.toString);new DateTime(m.tournament.tournamentTime.getTime()).getYear() >= 2010 }
