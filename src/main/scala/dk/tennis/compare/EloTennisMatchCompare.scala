 ...
 */
class EloTennisMatchCompare(atpMatchLoader: ATPMatchesLoader, histDataInMonths: Int = 12) extends TennisPlayerCompare {
  ...
  def matchProb(fullNamePlayerA: String, fullNamePlayerB: String, surface: SurfaceEnum, matchType: MatchTypeEnum, marketTime: Date): Double = {
    ...
    val matchProb = GenericEloRating.calcExpectedScore(playerARating, playerBRating)
    matchProb
  }
